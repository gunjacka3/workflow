	...
	public ObjectNode getHighlighted(@PathVariable String processInstanceId) {
	  ...
	  ObjectNode responseJSON = objectMapper.createObjectNode();
		...
		try {
			...
			if(!"".equals(pdid)){
//				pdid=processInstance.getProcessDefinitionId();
				ProcessDefinitionEntity processDefinition = (ProcessDefinitionEntity) repositoryService.getProcessDefinition(pdid);
				
				responseJSON.put("processDefinitionId", pdid);
				...
				pdid=historicProcessInstance.getProcessDefinitionId();
				
				ProcessDefinitionEntity processDefinition = (ProcessDefinitionEntity) repositoryService.getProcessDefinition(pdid);
				
				responseJSON.put("processDefinitionId", pdid);
				List<HistoricActivityInstance> has=historyService.createHistoricActivityInstanceQuery().processInstanceId(processInstanceId).list();
				
				for(HistoricActivityInstance ha:has){
					activitiesArray.add(ha.getActivityId());
				}
